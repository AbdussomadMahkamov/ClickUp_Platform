...
//                    return new ApiResponse("Kirishga huquq mavjud emas!", false);
//                }
//                Optional<WorkSpaceUsers> byUsersId = workSpaceUsersRepository.findByUsersId(principal.getId());
//                return new ApiResponse("Profilga xush kelibsiz "+byUsernameAndEmailKod.get().getFish()+"!\n"+tokenGenerate.TokenOlish(principal.getUsername(), byUsersId.get().getWorkSpaceLavozimlari()),true);

                return new ApiResponse("Profilga xush kelibsiz "+byUsernameAndEmailKod.get().getFish()+"!\n"+tokenGenerate.TokenOlish(principal.getUsername(), principal.getPlatformaLavozimlari()),true);
            }
